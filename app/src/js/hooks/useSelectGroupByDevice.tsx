...
const useSelectGroupByDevice = (devicePath = "device") => {
    const getDevice = (option: OptionInterface) => getIn(option, devicePath, undefined);
    const getDeviceType = (option: OptionInterface) => getIn(option, devicePath + ".deviceType", undefined);
    ...
    const getSelectedOptionLabel = (option: string | OptionInterface) => {
        ...
        const device = getDevice(option);
        const deviceType = getDeviceType(option);
        return getOptionLabel(option) + " (" + deviceType?.representation + " > " + device?.representation + ")";
    };
    ...
    const groupBy = (option: OptionInterface) => {
        const device = getDevice(option);
        const deviceType = getDeviceType(option);
        ...
        return device.representation + " (" + deviceType.representation + ")";
    };

    // Options has to be sorted to avoid duplicate group headers (MUI requirement)
    const sortOptions = (options: OptionsType) =>
        options.sort((optionA, optionB) => {
            const deviceA = getDevice(optionA);
            const deviceB = getDevice(optionB);

            if (!deviceA || !deviceB) {
                return 0;
            }

            return deviceA.representation.localeCompare(deviceB.representation);
        });
...
export default useSelectGroupByDevice;
