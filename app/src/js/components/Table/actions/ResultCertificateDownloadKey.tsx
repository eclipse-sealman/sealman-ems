...
type ResultCertificateDownloadKeyProps = Omit<ResultButtonDownloadProps, "endpoint"> &
    EntityButtonInterface &
    CertificateTypeProps;

const _denyKey = "downloadPrivateKey";
const isHidden = (deny?: DenyInterface) => isDenyHidden(_denyKey, ".accessDenied", deny);

const ResultCertificateDownloadKey = ({
    entityPrefix,
    certificateTypeId,
    denyKey = _denyKey,
    deny,
    ...props
}: ResultCertificateDownloadKeyProps) => {
    if (isHidden(deny)) {
        return null;
    }
    ...
    return (
        <ResultButtonDownload
            {...{
                label: "certificateDownload.action.downloadKey",
                denyKey,
                deny,
                variant: "contained",
                color: "info",
                ...
                endpoint: (result: ResultInterface) =>
                    "/" + entityPrefix + "/" + result.id + "/" + certificateTypeId + "/download/private",
                ...props,
            }}
        />
    );
...
export default ResultCertificateDownloadKey;
export { ResultCertificateDownloadKeyProps, isHidden };
