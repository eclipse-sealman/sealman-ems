// Copyright (c) 2025 Contributors to the Eclipse Foundation.
//
// See the NOTICE file(s) distributed with this work for additional
// information regarding copyright ownership.
//
// This program and the accompanying materials are made available under the
// terms of the Apache License, Version 2.0 which is available at
// https://www.apache.org/licenses/LICENSE-2.0
//
// SPDX-License-Identifier: Apache-2.0

import React from "react";
import CrudFormView, { CrudFormViewProps } from "~app/views/CrudFormView";
import DeviceTypeFieldsetContent, {
    DeviceTypeFieldsetContentProps,
} from "~app/fieldsets/content/DeviceTypeFieldsetContent";

type DeviceTypeFieldsetProps = DeviceTypeFieldsetContentProps & Omit<CrudFormViewProps, "children">;

const DeviceTypeFieldset = ({
    fields,
    disableAll,
    enableConfigMinRsrp,
    enableFirmwareMinRsrp,
    noCommunicationFields,
    ...formViewProps
}: DeviceTypeFieldsetProps) => {
    return (
        <CrudFormView {...formViewProps}>
            <DeviceTypeFieldsetContent
                {...{ fields, disableAll, enableConfigMinRsrp, enableFirmwareMinRsrp, noCommunicationFields }}
            />
        </CrudFormView>
    );
};

export default DeviceTypeFieldset;
export { DeviceTypeFieldsetProps };
