...
const BatchDisable = (props: BatchDisableProps) => {
    ...
    if (isAccessGranted({ adminScep: true }) && useableCertificateTypes !== undefined) {
        fields = {
            certificateBehaviours: <BatchUsersCertificateAutomaticBehaviorCollection {...{ enable: false }} />,
        };
    }
    ...
    return (
        <BatchFormMultiAlert
            {...{
                label: "batch.user.disable.action",
                ...props,
                dialogProps: {
                    title: "batch.user.disable.title",
                    label: hasUseableCertificateTypes()
                        ? "batch.user.disable.labelWithRevoke"
                        : "batch.user.disable.label",
                    formProps: {
