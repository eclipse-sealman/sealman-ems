...
const BatchEnable = (props: BatchEnableProps) => {
    ...
    if (isAccessGranted({ adminScep: true }) && useableCertificateTypes !== undefined) {
        fields = {
            certificateBehaviours: <BatchDevicesCertificateAutomaticBehaviorCollection {...{ enable: true }} />,
        };
    }
    ...
    return (
        <BatchFormMultiAlert
            {...{
                ...
                ...props,
                dialogProps: {
                    title: "batch.device.enable.title",
                    label: hasUseableCertificateTypes()
                        ? "batch.device.enable.labelWithGenerate"
                        : "batch.device.enable.label",
                    formProps: {
