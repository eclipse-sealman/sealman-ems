...
trait ApiCertificatesDownloadCaTrait
{
    ...
    public function executeDownloadCaAction(Request $request, int $id, int $certificateTypeId)
    {
        $usableCertificate = $this->getUsableCertificate($id, $certificateTypeId, CertificateDenyInterface::DOWNLOAD_CA_CERTIFICATE);

        $filename = 'ca-certificate';
        if ($usableCertificate->getCertificate()->getCertificateCaSubject()) {
            $filename = 'ca-'.$usableCertificate->getCertificate()->getCertificateCaSubject();
        }

        $filename .= '.crt';
        $content = $this->certificateManager->getCertificateCa($usableCertificate->getCertificate());

        if (!$content) {
            throw new RequestExecutionException('error.certificate.noCertificateCa');
        }
