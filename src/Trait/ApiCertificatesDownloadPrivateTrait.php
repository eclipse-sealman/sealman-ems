...
trait ApiCertificatesDownloadPrivateTrait
{
    ...
    public function executeDownloadPrivateAction(Request $request, int $id, int $certificateTypeId)
    {
        $usableCertificate = $this->getUsableCertificate($id, $certificateTypeId, CertificateDenyInterface::DOWNLOAD_PRIVATE_KEY);
        ...
        $filename .= '.key';
        $content = $this->certificateManager->getPrivateKey($usableCertificate->getCertificate());

        if (!$content) {
            throw new RequestExecutionException('error.certificate.noPrivateKey');
        }
        ...
        $response->headers->set('Content-Type', 'application/pkcs8');
