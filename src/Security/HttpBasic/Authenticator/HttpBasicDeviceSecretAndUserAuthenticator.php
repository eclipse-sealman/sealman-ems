...
class HttpBasicDeviceSecretAndUserAuthenticator extends AbstractHttpBasicAuthenticator
{
    public function createPassport(Request $request, string $username, string $password): Passport
    {
        ...
        $deviceSecretPassport = $this->createDeviceSecretPassport($request, $username, $password);
        if ($deviceSecretPassport) {
            return $deviceSecretPassport;
        }

        // Authentication using deviceSecret failed try Device User
