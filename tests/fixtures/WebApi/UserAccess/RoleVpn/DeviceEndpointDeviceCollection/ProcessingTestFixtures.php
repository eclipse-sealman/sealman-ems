...
use Tests\Suites\WebApi\UserAccess\RoleVpn\DeviceEndpointDeviceCollection\ProcessingTest;
...
use Tests\Utilities\WebApi\UserAccess\ProcessingTestCase;  // TODO decide if this is good approach

class ProcessingTestFixtures extends AbstractFixtureGroupAsClass implements DependentFixtureInterface
{
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        foreach (ProcessingTest::getTestCases() as $key => $testCase) {
    ...
    protected function createDevice(ObjectManager $manager, string $name): Device
    {
        ...
        $device->setVirtualSubnetCidr(29);
    ...
    protected function createEndpointDevice(ObjectManager $manager, Device $device, null|AccessibleEndpointDevice|NonAccessibleEndpointDevice $element, AccessTag $accessibleAccessTag)
    {
        ...
        $endpointDevice->setName(ProcessingTestCase::getElementOriginalName($key));
        $endpointDevice->setPhysicalIp('1.1.1.'.$key);
